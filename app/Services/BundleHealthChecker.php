...
class BundleHealthChecker
{
    ...
    public function evaluateBundle(array $bundle): array
    {
        ...
        $remote = match ($profile) {
            'hysteria' => $this->fetchHysteriaHealthFlags($bundle, $clientPort),
            'home' => $this->fetchHomeHealthFlags($bundle, $clientPort),
            default => $this->fetchVlessHealthFlags($bundle, $clientPort),
        };
        if ($remote === null) {
            $base['ssh_ok'] = false;
            $base['online'] = false;
...
BASH;
     ...
     * @return array{default_route: bool, listen_443: bool, xray: bool, egress_https: bool}|null
     */
    private function fetchHomeHealthFlags(array $bundle, int $clientPort): ?array
    {
        $script = <<<'BASH'
p="${1:-443}"
default_route=0
